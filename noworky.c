...
{
	unsigned int i;
	// CHANGED: changed condition statement because for unsigned int, it is never negative
	// 			so having the condition i >= 0 is always going to be true and allows it to
	//			reach negative values and will be accessing out-of-bounds
	for (i = size-1; i != -1; i--) {
...
int main()
{
	...
	for (i=0; i < NUM_ELEMENTS; i++) { // CHANGED: initialize within for loop to ensure it is 0
		source[i] = i * 2.0;
		target[i] = i * 10.0;
	}
